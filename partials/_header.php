<header class="header">
  <div class="wrapper">
    <div class="header__logo-container">
      <img src="./assets/img/logo.svg" alt="Zeta Soft logo" class="header__logo">
    </div>
    <ul class="header__menu menu hide-md hide-sm">
      <li class="menu__item"><a href="#" class="header__link">Услуги</a></li>
      <li class="menu__item"><a href="#" class="header__link">Продукты</a></li>
      <li class="menu__item hide-bg"><a href="#" class="header__link">Компетенции</a></li>
      <li class="menu__item"><a href="#" class="header__link">Кейсы</a></li>
      <li class="menu__item hide-bg"><a href="#" class="header__link">Блог</a></li>
      <li class="menu__item"><a href="#" class="header__link">О компании</a></li>
    </ul>
    <a href="tel:+7 812 490-67-89" class="header__phone header__link">
      <span class="hide-sm hide-md">+7 812 490-67-89</span>
      <span class="icon icon--phone show-sm"></span>
      <!-- TODO: кнопки телефона и входа на мобилке поменяны местами, это норм? -->
    </a>
    <div class="header__controls">
      <button class="button button--link header__link">Войти<span class="icon icon--login"></span></button>
      <button class="burger show-md"></button>
    </div>
  </div>
</header>
